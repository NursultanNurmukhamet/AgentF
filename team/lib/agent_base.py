...
def create_invoke_handler(
    ...
    fast_path_fn: Callable[[dict, int, int], list[dict] | None] | None = None,
):
    ...
    async def invoke(payload, context):
        try:
            ...
            # Optional deterministic path for latency-critical, unambiguous
            # decisions.  Returning None delegates the decision to the LLM.
            if fast_path_fn is not None:
                try:
                    commands = fast_path_fn(game_state, team_id, effective_pid)
                    if commands:
                        log.info(f"Fast path returned {[c.get('commandType') for c in commands]}")
                        yield json.dumps(commands)
                        return
                except Exception as fast_path_error:
                    log.warn(f"{position_label} fast path failed: {fast_path_error}; using LLM")
