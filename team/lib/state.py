...
def get_possession_player(ball: dict, players: list):
    """Return the player object that currently holds the ball.

    The current game payload uses position-local identifiers (``agentId_0`` ..
    ``agentId_4``) for both teams, so the identifier alone is ambiguous.  When
    both teams have the same numeric id, the holder is the matching player
    closest to the ball position.
    """
    possession_id = _possession_idx(ball)
    if possession_id is None:
        return None

    matches = [p for p in players if _player_idx(p) == possession_id]
    if not matches:
        return None
    if len(matches) == 1:
        return matches[0]
    ...
    return min(matches, key=lambda p: dist(p.get("position", {}), ball_pos))


def stamina_percent(value) -> float:
    """Normalise either a 0..1 or 0..100 stamina value to percent."""
    try:
        stamina = float(value)
    ...
    if 0.0 <= stamina <= 1.0:
        stamina *= 100.0
    return max(0.0, min(100.0, stamina))
...
def summarize_state(
    ...
    if me:
        pos = me.get("position", {})
        stam = stamina_percent(me.get("stamina", 100))
        dist_ball = dist(pos, ball_pos)
        has_ball = we_have_ball and possession_id == my_player_id
